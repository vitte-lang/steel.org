...
var TOP_LEVEL_BLOCKS = ["workspace", "profile", "tool", "bake", "run", "export"];
var DIRECTIVES = [".set", ".exec", ".make", ".output", ".ref", ".needs", ".takes", ".emits"];
var OUTPUT_PORTS = ["exe", "lib", "obj", "out"];
var MAKE_KINDS = ["glob", "cglob"];
var WORKSPACE_KEYS = ["name", "root", "target_dir", "profile"];
var PROFILE_KEYS = ["opt", "debug"];
var PROFILE_VALUES = {
  opt: ["0", "1", "2", "3"],
  debug: ["0", "1"]
};
var WORKSPACE_VALUES = {
...
var VARIABLE_HINTS = ["${root}", "${target_dir}", "${profile}", "${name}"];
var OUTPUT_PREFIXES = ["target/out", "dist", "build", "bin"];
var MAX_WORKSPACE_FILES = 200;
var MAX_DEPTH = 6;
var UNKNOWN_BLOCK_CODE = "unknown-block";
var UNKNOWN_DIRECTIVE_CODE = "unknown-directive";
var MISSING_BLOCK_CLOSE_CODE = "missing-block-close-bracket";
var UNEXPECTED_BLOCK_END_CODE = "unexpected-block-end";
...
connection.onInitialize((params) => {
  workspaceFolders = params.workspaceFolders?.map((f) => f.uri.replace("file://", "")) ?? [];
...
connection.onCompletion((params) => {
  ...
  if (/\.output\b/.test(line)) {
    ...
    for (const prefix of OUTPUT_PREFIXES) {
      items.push({
        label: `"${prefix}/"`,
        kind: import_node.CompletionItemKind.Folder,
        insertText: `"${prefix}/"`
      });
    }
    return items;
  ...
  if (/\.emits\b/.test(line)) {
    ...
    if (/\bas\s+\"[^\"]*$/.test(line)) {
      items.push({
        label: '"-o"',
        kind: import_node.CompletionItemKind.Constant,
        insertText: '"-o"'
      });
      return items;
    }
  ...
  if (/\.set\b/.test(line)) {
    const keys = blockKind === "profile" ? PROFILE_KEYS : blockKind === "workspace" ? WORKSPACE_KEYS : [];
...
connection.onHover((params) => {
  ...
  const value = suggestions.length > 0 ? `${hit.message}

Quick fix: ${suggestions.join(" | ")}` : hit.message;
  return {
    contents: {
      kind: "markdown",
      value
...
function collectSymbols(text, extraSources) {
  const bakes = /* @__PURE__ */ new Set();
  ...
  while (stack.length > 0 && out.length < MAX_WORKSPACE_FILES) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (entry.name === "steelconf" || entry.name.endsWith(".steelconf") || entry.name === "steelconfig.mff") {
...
function currentBlockKind(before) {
  const stack = [];
...
function makeDiag(line, start, end, message, severity, code, data) {
  return {
    severity,
    range: {
...
function findBakeBlockCloseLine(lines, bakeName) {
...
function hoverSuggestions(diag) {
  if (diag.code === "unknown-tool" && typeof diag.data === "string") {
